...
# Импорт MOSCOW_TZ оставлен для потенциального использования, хотя в коде используется UTC
...
class AsyncDatabase:
    ...
    async def init(self):
        ...
        ФИКС: Удален лишний async with self.conn: для предотвращения ошибки "threads can only be started once".
        """
    ...
    async def set_temp_session(self, user_id: int, phone: Optional[str] = None, qr_data: Optional[str] = None) -> bool:
        """
        ✅ ФИНАЛЬНЫЙ ФИКС: Обеспечивает корректное соответствие полей и плейсхолдеров.
        """
        fields_to_update = []
        values = [user_id] # user_id всегда первый в values
        
        if phone is not None:
            fields_to_update.append("phone")
            values.append(phone)
        if qr_data is not None:
            fields_to_update.append("qr_login_data")
            values.append(qr_data)
        
        if not fields_to_update:
            return True
        
        # 1. Формируем ВСЕ имена столбцов для INSERT (user_id всегда присутствует)
        field_names = ', '.join(["user_id"] + fields_to_update) 
        
        # 2. Формируем ВСЕ плейсхолдеры для VALUES (соответствует длине списка values)
        placeholders = ', '.join(['?' for _ in values]) 
        
        # 3. Формируем SET-часть (только для обновляемых полей)
        update_clause = ', '.join([f"{f} = EXCLUDED.{f}" for f in fields_to_update])
        
        query = f"""
            INSERT INTO temp_sessions ({field_names}) 
            VALUES ({placeholders})
